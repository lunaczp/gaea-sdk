...
class MessageSet implements \Iterator
{
     ...
     * partition object
     *
     * @var \Gaea\Storage\Kafka\Protocol\Fetch\Partition
     * @access private
     */
     ...
     * @access public
     */
    public function __construct(\Gaea\Storage\Kafka\Protocol\Fetch\Partition $partition, $context = array())
    {
        $this->stream = $partition->getStream();
        $this->partition = $partition;
        $this->context   = $context;
        $this->messageSetSize = $this->getMessageSetSize();
        \Gaea\Storage\Kafka\Log::log("messageSetSize: {$this->messageSetSize}", LOG_INFO);
    }
    ...
    public function valid()
    {
        if (!$this->valid && $this->messageSetSize != 0) {
            ...
            \Gaea\Storage\Kafka\Protocol\Fetch\Helper\Helper::onPartitionEof($this->partition);
        }
    ...
    protected function getMessageSetSize()
    {
        ...
        if ($size < 0) {
            throw new \Gaea\Storage\Kafka\Exception\OutOfRange($size . ' is not a valid message size');
        }
    ...
    public function loadNextMessage()
    {
        ...
        try {
            ...
            \Gaea\Storage\Kafka\Log::log("already fetch: {$this->validByteCount}, {$e->getMessage()}", LOG_INFO);
            return false;
        }
