...
class Helper
{
     ...
     *
     * @param string $key
     * @param \Gaea\Storage\Kafka\Protocol\Fetch\Helper\HelperAbstract $helper
     * @static
     * @access public
     * @return void
     */
    ...
    {
        if (is_null($helper)) {
            $className = '\\Gaea\\Storage\\Kafka\\Protocol\\Fetch\\Helper\\' . $key;
            if (!class_exists($className)) {
                throw new \Gaea\Storage\Kafka\Exception('helper is not exists.');
            }
            $helper = new $className();
        }

        if ($helper instanceof \Gaea\Storage\Kafka\Protocol\Fetch\Helper\HelperAbstract) {
            self::$helpers[$key] = $helper;
        } else {
            throw new \Gaea\Storage\Kafka\Exception('this helper not instance of `\Gaea\Storage\Kafka\Protocol\Fetch\Helper\HelperAbstract`');
        }
    }
