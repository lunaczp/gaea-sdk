...
class Topic implements \Iterator, \Countable
{
     ...
     * __construct
     *
     * @param \Gaea\Storage\Kafka\Socket|array $streams
     * @param array $context
     * @access public
     */
    ...
    protected function getTopicCount()
    {
        ...
        foreach (array_values($this->streams) as $key => $stream) {
            ...
            if ($count <= 0) {
                throw new \Gaea\Storage\Kafka\Exception\OutOfRange($count . ' is not a valid topic count');
            }
        }
    ...
    public function loadNextTopic()
    {
        if ($this->validCount >= $this->topicCount) {
            \Gaea\Storage\Kafka\Protocol\Fetch\Helper\Helper::onStreamEof($this->currentStreamLockKey);
            return false;
        }
