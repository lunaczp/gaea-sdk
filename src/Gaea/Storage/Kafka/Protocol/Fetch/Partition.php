...
namespace Gaea\Storage\Kafka\Protocol\Fetch;

use \Gaea\Storage\Kafka\Protocol\Decoder;
...
class Partition implements \Iterator, \Countable
{
     ...
     * __construct
     *
     * @param \Gaea\Storage\Kafka\Protocol\Fetch\Topic $topic
     * @param array $context
     * @access public
     */
    public function __construct(\Gaea\Storage\Kafka\Protocol\Fetch\Topic $topic, $context = array())
    {
     ...
     *
     * @access public
     * @return \Gaea\Storage\Kafka\Socket
     */
    public function getStream()
    {
        return $this->stream;
    ...
    protected function getPartitionCount()
    {
        ...
        if ($count <= 0) {
            throw new \Gaea\Storage\Kafka\Exception\OutOfRange($count . ' is not a valid partition count');
        }
    ...
    public function loadNextPartition()
    {
        ...
        try {
            ...
            \Gaea\Storage\Kafka\Log::log("kafka client:fetch partition:" . $partitionId, LOG_INFO);

            $errCode = $this->stream->read(2, true);
            $errCode = Decoder::unpack(Decoder::BIT_B16, $errCode);
            $this->errCode = array_shift($errCode);
            if ($this->errCode != 0) {
                throw new \Gaea\Storage\Kafka\Exception(\Gaea\Storage\Kafka\Protocol\Decoder::getError($this->errCode));
            }
            $offset = $this->stream->read(8, true);
            $this->offset  = \Gaea\Storage\Kafka\Protocol\Decoder::unpack(Decoder::BIT_B64, $offset);

            $this->key = $partitionId;
            $this->current = new MessageSet($this, $this->context);
        } catch (\Gaea\Storage\Kafka\Exception $e) {
            \Gaea\Storage\Kafka\Log::log($e->getMessage(), LOG_ERR);
            return false;
        }
