...
class Decoder extends Protocol
{
    ...
    public function produceResponse()
    {
        ...
        if (!$dataLen) {
            throw new \Gaea\Storage\Kafka\Exception\Protocol('produce response invalid.');
        }
    ...
    public function fetchResponse()
    {
        return new \Gaea\Storage\Kafka\Protocol\Fetch\Topic($this->stream);
    }
    ...
    public function metadataResponse()
    {
        ...
        $dataLen = array_shift($dataLen);
        if (!$dataLen) {
            throw new \Gaea\Storage\Kafka\Exception\Protocol('metaData response invalid.');
        }
        $data = $this->stream->read($dataLen, true);
        $offset = 4;
    ...
    public function offsetResponse()
    {
        ...
        $dataLen = array_shift($dataLen);
        if (!$dataLen) {
            throw new \Gaea\Storage\Kafka\Exception\Protocol('offset response invalid.');
        }
        $data = $this->stream->read($dataLen, true);
        $offset = 4;
        $topicCount = self::unpack(self::BIT_B32, substr($data, $offset, 4));
    ...
    public function commitOffsetResponse()
    {
        ...
        $dataLen = array_shift($dataLen);
        if (!$dataLen) {
            throw new \Gaea\Storage\Kafka\Exception\Protocol('commit offset response invalid.');
        }
        $data = $this->stream->read($dataLen, true);
        $offset = 4;
        $topicCount = self::unpack(self::BIT_B32, substr($data, $offset, 4));
    ...
    public function fetchOffsetResponse()
    {
        ...
        $dataLen = array_shift($dataLen);
        if (!$dataLen) {
            throw new \Gaea\Storage\Kafka\Exception\Protocol('fetch offset response invalid.');
        }
        $data = $this->stream->read($dataLen, true);
        $offset = 4;
        $topicCount = self::unpack(self::BIT_B32, substr($data, $offset, 4));
