...
abstract class Protocol
{
     ...
     * __construct
     *
     * @param \Gaea\Storage\Kafka\Socket $stream
     * @access public
     */
    public function __construct(\Gaea\Storage\Kafka\Socket $stream)
    {
        $this->stream = $stream;
    }
    ...
    protected static function checkLen($type, $bytes)
    {
        ...
        if (strlen($bytes) != $len) {
            throw new \Gaea\Storage\Kafka\Exception\Protocol('unpack failed. string(raw) length is ' . strlen($bytes) . ' , TO ' . $type);
        }
    }
