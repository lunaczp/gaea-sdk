...
class Offset
{
    ...
    public function __construct($client, $groupId, $topicName, $partitionId = 0)
    {
        ...
        $this->encoder = new \Gaea\Storage\Kafka\Protocol\Encoder($conn);
        $this->decoder = new \Gaea\Storage\Kafka\Protocol\Decoder($conn);
    }
    ...
    public function setOffset($offset)
    {
        ...
        if ($offset > $maxOffset) {
            throw new \Gaea\Storage\Kafka\Exception('this offset is invalid. must less than max offset:' . $maxOffset);
        }
        ...
        if (!isset($result[$topicName][$partitionId]['errCode'])) {
            throw new \Gaea\Storage\Kafka\Exception('commit topic offset failed.');
        }
        if ($result[$topicName][$partitionId]['errCode'] != 0) {
            throw new \Gaea\Storage\Kafka\Exception(\Gaea\Storage\Kafka\Protocol\Decoder::getError($result[$topicName][$partitionId]['errCode']));
        }
    }
    ...
    public function getOffset($defaultOffset = self::DEFAULT_LAST)
    {
        ...
        $partitionId = $this->partitionId;
        if (!isset($result[$topicName][$partitionId]['errCode'])) {
            throw new \Gaea\Storage\Kafka\Exception('fetch topic offset failed.');
        }
    ...
    public function getProduceOffset($timeLine = self::LAST_OFFSET)
    {
        ...
        if (!isset($result[$topicName][$partitionId]['offset'])) {
            if (isset($result[$topicName][$partitionId]['errCode'])) {
                ...
                throw new \Gaea\Storage\Kafka\Exception('get offset failed. topic name:' . $this->topicName . ' partitionId: ' . $this->partitionId);
            }
        }
