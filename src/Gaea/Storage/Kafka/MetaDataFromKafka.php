...
class MetaDataFromKafka implements ClusterMetaData
{
    ...
    /**
     * @var \Gaea\Storage\Kafka\Client $client
     * @access public
     * @return void
     */
    public function setClient(\Gaea\Storage\Kafka\Client $client)
    {
        $this->client = $client;
    }
    ...
    {
        if ($this->client === null) {
            throw new \Gaea\Storage\Kafka\Exception('client was not provided');
        }
        $response = null;
        foreach ($this->hostList as $host) {
            try {
                ...
                $decoder = new \Gaea\Storage\Kafka\Protocol\Decoder($conn);
        ...
        if ($response) {
            ...
            throw new \Gaea\Storage\Kafka\Exception('Could not connect to any kafka brokers');
        }
    }
