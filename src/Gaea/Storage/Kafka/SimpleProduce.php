...
class SimpleProduce
{
    ...
    {
        if ($hostList instanceof \Gaea\Storage\Kafka\ClusterMetaData) {
            $metadata = $hostList;
        } elseif ($kafkaHostList !== null) {
            $metadata = new \Gaea\Storage\Kafka\MetaDataFromKafka($kafkaHostList);
        } else {
            $metadata = new \Gaea\Storage\Kafka\ZooKeeper($hostList, $timeout);
        }
        $this->client = new \Gaea\Storage\Kafka\Client($metadata);
    }
    // }}}
