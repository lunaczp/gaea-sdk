...
class Consumer
{
     ...
     * produce instance
     *
     * @var \Gaea\Storage\Kafka\Produce
     * @access private
     */
     ...
     * @var integer
     */
    private $offsetStrategy = \Gaea\Storage\Kafka\Offset::DEFAULT_EARLY;
    ...
    {
        $zookeeper = new \Gaea\Storage\Kafka\ZooKeeper($hostList, $timeout);
        $this->client = new \Gaea\Storage\Kafka\Client($zookeeper);
    }
    ...
    public function setPartition($topicName, $partitionId = 0, $offset = null)
    {
        if (is_null($offset)) {
            if ($this->fromOffset) {
                $offsetObject = new \Gaea\Storage\Kafka\Offset($this->client, $this->group, $topicName, $partitionId);
                $offset = $offsetObject->getOffset($this->offsetStrategy);
                \Gaea\Storage\Kafka\Log::log('topic name:' . $topicName . ', part:' . $partitionId . 'get offset from kafka server, offet:' . $offset, LOG_DEBUG);
            } else {
                $offset = 0;
            }
        }
     ...
     *
     * @access public
     * @return \Gaea\Storage\Kafka\Protocol\Fetch\Topic|bool
     */
    public function fetch()
    {
        ...
        foreach ($data as $host => $requestData) {
            ...
            $conn    = $connArr['stream'];
            $encoder = new \Gaea\Storage\Kafka\Protocol\Encoder($conn);
            $encoder->fetchRequest($requestData);
            $streams[$connArr['key']] = $conn;
            $this->client->freeStream($connArr['key']);
        }

        $fetch = new \Gaea\Storage\Kafka\Protocol\Fetch\Topic($streams, $data);

        // register fetch helper
        $freeStream = new \Gaea\Storage\Kafka\Protocol\Fetch\Helper\FreeStream($this->client);
        $freeStream->setStreams($streams);
        \Gaea\Storage\Kafka\Protocol\Fetch\Helper\Helper::registerHelper('freeStream', $freeStream);

        // register partition commit offset
        $commitOffset = new \Gaea\Storage\Kafka\Protocol\Fetch\Helper\CommitOffset($this->client);
        $commitOffset->setGroup($this->group);
        \Gaea\Storage\Kafka\Protocol\Fetch\Helper\Helper::registerHelper('commitOffset', $commitOffset);

        $updateConsumer = new \Gaea\Storage\Kafka\Protocol\Fetch\Helper\Consumer($this);
        \Gaea\Storage\Kafka\Protocol\Fetch\Helper\Helper::registerHelper('updateConsumer', $updateConsumer);
