...
class Socket
{
    ...
    public function connect()
    {
        ...
        if (empty($this->host)) {
            throw new \Gaea\Storage\Kafka\Exception('Cannot open null host.');
        }
        if ($this->port <= 0) {
            throw new \Gaea\Storage\Kafka\Exception('Cannot open without port.');
        }
        ...
        if ($this->stream == false) {
            $error = 'Could not connect to '
                    . $this->host . ':' . $this->port
                    . ' ('.$errstr.' ['.$errno.'])';
            throw new \Gaea\Storage\Kafka\Exception\SocketConnect($error);
        }
     ...
     * @throws \Gaea\Storage\Kafka\Exception\SocketEOF
     */
    public function read($len, $verifyExactLength = false)
    {
        if ($len > self::READ_MAX_LEN) {
            throw new \Gaea\Storage\Kafka\Exception\SocketEOF('Could not read '.$len.' bytes from stream, length too longer.');
        }
        ...
        if ($readable > 0) {
            ...
            while ($remainingBytes > 0) {
                ...
                if ($chunk === false) {
                    $this->close();
                    throw new \Gaea\Storage\Kafka\Exception\SocketEOF('Could not read '.$len.' bytes from stream (no data)');
                }
                if (strlen($chunk) === 0) {
                    // Zero bytes because of EOF?
                    if (feof($this->stream)) {
                        $this->close();
                        throw new \Gaea\Storage\Kafka\Exception\SocketEOF('Unexpected EOF while reading '.$len.' bytes from stream (no data)');
                    }
                    // Otherwise wait for bytes
                    $readable = @stream_select($read, $null, $null, $this->recvTimeoutSec, $this->recvTimeoutUsec);
                    if ($readable !== 1) {
                        throw new \Gaea\Storage\Kafka\Exception\SocketTimeout('Timed out reading socket while reading ' . $len . ' bytes with ' . $remainingBytes . ' bytes to go');
                    }
            ...
            if ($len === $remainingBytes || ($verifyExactLength && $len !== strlen($data))) {
                ...
                $this->close();
                throw new \Gaea\Storage\Kafka\Exception\SocketEOF('Read ' . strlen($data) . ' bytes instead of the requested ' . $len . ' bytes');
            }
        ...
        if (false !== $readable) {
            ...
            if (!empty($res['timed_out'])) {
                $this->close();
                throw new \Gaea\Storage\Kafka\Exception\SocketTimeout('Timed out reading '.$len.' bytes from stream');
            }
        }
        $this->close();
        throw new \Gaea\Storage\Kafka\Exception\SocketEOF('Could not read '.$len.' bytes from stream (not readable)');
    }
        ...
        while ($written < $buflen) {
            ...
            if ($writable > 0) {
                ...
                if ($wrote === -1 || $wrote === false) {
                    throw new \Gaea\Storage\Kafka\Exception\Socket('Could not write ' . strlen($buf) . ' bytes to stream, completed writing only ' . $written . ' bytes');
                } elseif ($wrote === 0) {
                    // Increment the number of times we have failed
                    $failedWriteAttempts++;
                    if ($failedWriteAttempts > $this->maxWriteAttempts) {
                        throw new \Gaea\Storage\Kafka\Exception\Socket('After ' . $failedWriteAttempts . ' attempts could not write ' . strlen($buf) . ' bytes to stream, completed writing only ' . $written . ' bytes');
                    }
                } else {
            ...
            if (false !== $writable) {
                $res = stream_get_meta_data($this->stream);
                if (!empty($res['timed_out'])) {
                    throw new \Gaea\Storage\Kafka\Exception\SocketTimeout('Timed out writing ' . strlen($buf) . ' bytes to stream after writing ' . $written . ' bytes');
                }
            }
            throw new \Gaea\Storage\Kafka\Exception\Socket('Could not write ' . strlen($buf) . ' bytes to stream');
        }
        return $written;
    }
